DECLARE
    -- Curseur pour récupérer toutes les tables de l'utilisateur actuel
    CURSOR c_tables IS
        SELECT table_name
        FROM user_tables
        ORDER BY table_name;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('=== Suppression de toutes les tables ===');
    DBMS_OUTPUT.PUT_LINE('Utilisateur: ' || USER);
    DBMS_OUTPUT.PUT_LINE('Nombre total de tables à supprimer: ' || v_total);
    DBMS_OUTPUT.PUT_LINE('');
    
    IF v_total = 0 THEN
        DBMS_OUTPUT.PUT_LINE('Aucune table à supprimer.');
        RETURN;
    END IF;
    ...
    FOR rec_table IN c_tables LOOP
        BEGIN
            -- Supprimer la table avec CASCADE CONSTRAINTS pour éviter les erreurs de dépendances
            v_sql := 'DROP TABLE ' || rec_table.table_name || ' CASCADE CONSTRAINTS';
            EXECUTE IMMEDIATE v_sql;
            
            v_count := v_count + 1;
            DBMS_OUTPUT.PUT_LINE('✓ Table supprimée: ' || rec_table.table_name);
            
        EXCEPTION
            WHEN OTHERS THEN
                DBMS_OUTPUT.PUT_LINE('✗ Erreur lors de la suppression de ' || rec_table.table_name || ': ' || SQLERRM);
        END;
    END LOOP;
    
    DBMS_OUTPUT.PUT_LINE('');
    DBMS_OUTPUT.PUT_LINE('=== Résumé ===');
    DBMS_OUTPUT.PUT_LINE('Tables supprimées avec succès: ' || v_count || '/' || v_total);
    ...
    ELSE
        DBMS_OUTPUT.PUT_LINE('⚠ Il reste ' || v_total || ' table(s) non supprimée(s)');
    END IF;
...
DECLARE
    -- Curseur pour récupérer toutes les tables de l'utilisateur bdjeuvideo
    CURSOR c_tables IS
        SELECT table_name
        FROM all_tables
        WHERE owner = 'BDJEUVIDEO'
        ORDER BY table_name;
    
    -- Variables
    v_sql VARCHAR2(4000);
    v_count NUMBER;
    v_total_copied NUMBER := 0;
    
BEGIN
    DBMS_OUTPUT.PUT_LINE('=== Début de la copie des données ===');
    DBMS_OUTPUT.PUT_LINE('Source: BDJEUVIDEO');
    DBMS_OUTPUT.PUT_LINE('Destination: ' || USER);
    DBMS_OUTPUT.PUT_LINE('');
    
    -- Parcourir toutes les tables
    FOR rec_table IN c_tables LOOP
        BEGIN
            -- Vider la table de destination avant la copie
            v_sql := 'DELETE FROM ' || rec_table.table_name;
            EXECUTE IMMEDIATE v_sql;
            
            -- Copier les données
            v_sql := 'INSERT INTO ' || rec_table.table_name || 
                     ' SELECT * FROM bdjeuvideo.' || rec_table.table_name;
            EXECUTE IMMEDIATE v_sql;
            
            -- Récupérer le nombre de lignes copiées
            v_count := SQL%ROWCOUNT;
            v_total_copied := v_total_copied + v_count;
            
            -- Valider la transaction pour cette table
            COMMIT;
            
            DBMS_OUTPUT.PUT_LINE('✓ ' || rec_table.table_name || ': ' || v_count || ' lignes copiées');
            
        EXCEPTION
            WHEN OTHERS THEN
                -- En cas d'erreur, annuler les modifications de cette table
                ROLLBACK;
                DBMS_OUTPUT.PUT_LINE('✗ Erreur sur ' || rec_table.table_name || ': ' || SQLERRM);
        END;
    END LOOP;
    
    DBMS_OUTPUT.PUT_LINE('');
    DBMS_OUTPUT.PUT_LINE('=== Fin de la copie ===');
    DBMS_OUTPUT.PUT_LINE('Total de lignes copiées: ' || v_total_copied);
